...
vec4 BP_brdf(float kl, float kg, float f, vec4 indir, vec4 normal, vec4 outdir) // the Blinn-Phong brdf 
{

	vec4 h=normalize(outdir+indir);

	return vec4(vec3(kl/3.14)*bunny_colour.xyz + vec3(((8+f)/(8*3.14)))*kg*pow(dot(normal,h),f), 1);
}
...
vec4 CT_brdf(vec4 normal, vec4 indir, vec4 outdir, vec4 f0, float alpha, float lval, float fval) //Cook-Torrance brdf
{
	...
	vec4 lambert = bunny_colour*diffuse_brdf();
...
void main () {
  ...
  for (int l = 0; l < light_count; ++l )
  {
	...
	if(gl_FragCoord.x > window_width/2)
	{
		...
		frag_colour = frag_colour + light_strength[l] * light_colour[l] * BP_brdf(kl_var, kg_var, f_var, l_in, n, outdir) * constrain(dot(l_in, n));
	}else
	{
		position_edit.x = position_edit.x;// - 0.5;
		l_in = normalize(light_position[l] - position_edit);
		outdir=normalize(position_edit-eyepos);
		frag_colour = frag_colour + light_strength[l] * light_colour[l] * CT_brdf(n, l_in, outdir, f0, alpha, slider_scale, 1-slider_scale)* constrain(dot(l_in, n));
	}
